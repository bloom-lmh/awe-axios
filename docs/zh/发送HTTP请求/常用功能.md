...
::: tip 默认值
当你使用`useRetry`包裹实际开发接口时，`axios-plus`会采设置默认的重试次数和基础延迟时间，其中：
...
上面的方式是最简单的，不需要提供任何配置项，`axios-plus`会以默认的配置项进行请求重发，但这并不一定能满足你的需求，所以`axios-plus`支持多种配置方式，你可以通过以下方式进行配置来满足你的项目需求：
...
::: tip 默认值
当你使用`useDebounce`包裹实际开发接口时，`axios-plus`会设置默认的延迟时间和是否立即执行：
...
::: tip 默认值
当你使用`useThrottle`包裹实际开发接口时，`axios-plus`会设置默认的间隔时间：`100`ms
:::
...
`axios-plus`支持多种功能组合，比如你的接口可以即具有防抖功能，又请求重发功能，或者即具有节流功能，又请求重发功能，你可以根据你的项目需求选择合适的组合。使用方法也很简单，如下所示：
...
`axios-plus`不推荐防抖与节流不能同时使用，因为这两个功能的实现方式是互相冲突的，可能会导致请求失败。
推荐组合如下
