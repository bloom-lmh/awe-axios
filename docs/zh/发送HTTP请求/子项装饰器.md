...
`axios-plus`定义了一些子项装饰器，这些装饰器就像是语法糖一样，可以让用户更加方便的去进行配置。我们来看一个案例：
...
可以看到我们所有的`http`请求配置项都配置再`@Get`中，这会导致配置十分臃肿，于是`axios-plus`定义了一些子项装饰器，用来让配置结构更加清晰，比如下面我们使用了`@TransformResponse`将响应数据处理器从`@Get`中分离：
...
实际上`axios-plus`还有其它的子项装饰器，比如：`@RefAxios`，`@AxiosRef`等
