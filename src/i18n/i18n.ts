...
export class I18n {
  ...
  // 翻译v1 直接传入对象
  static t(expression: string, params?: { [key: string]: string | undefined | null }): string {
    if (typeof expression !== 'string') {
      throw new Error(`i18n: 错误消息模板必须为字符串类型，请检查表达式是否正确（当前表达式值为：${expression}）`);
    }
    ...
    return Object.keys(params).reduce((result, key) => {
      return result.replace(new RegExp(`\\{\\{${key}\\}\\}`, 'g'), params[key] as string);
    }, expression);
  }

  // 翻译
  static t_v2(expression: string, params?: { [key: string]: string | undefined | null }): string {
    // 错误key
    let translation = expression.split('.').reduce((locale, key) => {
      if (!locale.hasOwnProperty(key)) {
        throw new Error(`i18n: 找不到翻译键: ${key} (当前语言: ${this.currentLocale.name}，请检查表达式是否正确`);
      }
      return locale[key];
